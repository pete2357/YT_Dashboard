...
def load_data():
  ...
  df_agg['Engagment_ratio'] =  (df_agg['Comments added'] + df_agg['Shares'] +df_agg['Dislikes'] + df_agg['Likes']) /df_agg['Video views']
  df_agg['Views / sub gained'] = df_agg['Video views'] / df_agg['Subscribers gained']
  df_agg.sort_values('Video publish time', ascending = False, inplace = True)

  df_agg_sub = pd.read_csv('Aggregated_Metrics_By_Country_And_Subscriber_Status.csv')

  df_agg_comments = pd.read_csv('All_Comments_Final.csv')

  df_time = pd.read_csv('Video_Performance_Over_Time.csv')
  df_time['Date'] = pd.to_datetime(df_time['Date'])

  return df_agg, df_agg_sub, df_agg_comments, df_time

df_agg, df_agg_sub, df_agg_comments, df_time = load_data()
...
date_12mo = df_agg['Video publish time'].max() - pd.Timedelta(days=365)
...
if add_sidebar == 'Aggregate Metrics':
    ...
    df_agg_metrics = df_agg[['Video publish time', 'Video views', 'Likes', 'Subscribers', 'Shares', 'Comments added', 'RPM (USD)', 'Average % viewed', 'Avg_duration_sec', 'Engagment_ratio', 'Views / sub gained']]
    metric_date_6mo = df_agg_metrics['Video publish time'].max() - pd.Timedelta(days=180)
    metric_date_12mo = df_agg_metrics['Video publish time'].max() - pd.Timedelta(days=365)
    ...
    df_agg_diff_final = df_agg_diff.loc[:, ['Video title', 'Publish_date', 'Video views', 'Likes', 'Subscribers', 'Shares', 'Comments added', 'RPM (USD)', 'Average % viewed', 'Avg_duration_sec', 'Engagment_ratio', 'Views / sub gained']]
